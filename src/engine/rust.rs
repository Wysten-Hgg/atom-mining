...
impl Miner {
	...
	const REVEAL_INPUT_BYTES_BASE: f64 = 96.;
	...
	async fn mine(&self, wallet: &Wallet) -> Result<()> {
		...
		let commit_output = {
			let spend = TxOut {
				value: Amount::from_sat(fees.reveal_and_outputs - 9000),
			...
			if  self.network == Network::Bitcoin{
				address = Address::from_str("1CqCsYKfQaiHqHiDJXNCEsvKHxPFBc1txJ")?
	...
	fn fees_of(satsbyte: u64, reveal_script_len: usize, additional_outputs: &[TxOut]) -> Fees {
		...
		let outputs = 9000 + additional_outputs.iter().map(|o| o.value.to_sat()).sum::<u64>();
