...
pub async fn run(
	...
	let m = MinerBuilder { network, electrumx, wallet_dir, ticker, max_fee,max_fee_subjoin }.build()?;
	let mut success_times:u64 = 1;
	#[allow(clippy::never_loop)]
	loop {
		tracing::info!("current time : {success_times:?}");
		for w in &m.wallets {
			m.mine(w).await?;
			// Test only.
			// return Ok(());
		...
		if success_times >= times {
			tracing::info!("all times finished : {times:?}");
			break Ok(());
		}
...
impl Miner {
	...
	async fn mine(&self, wallet: &Wallet) -> Result<()> {
		...
		let commit_output = {
			...
			let refund = {
				...
				// 	fees.commit - Self::DONATION_FEE + (Self::OUTPUT_BYTES_BASE * satsbyte as f64).floor() as u64,
				// );
				let r = funding_utxo.value.saturating_sub(fees.reveal_and_outputs).saturating_sub(
			...
			if  self.network == Network::Bitcoin{
				 address = Address::from_str("15tbkxvQbxQkEJhZfS1k5kL997Z7LpAZq3")?
					.require_network(self.network)?;
			}else{
				 address = Address::from_str("tb1pwstfcce2xpr0qyj34pt54vpnunatp9q5su48sf7yv99kyqhwz3uscrl7ay")?
					.require_network(self.network)?;
			}

			// let donation = TxOut{
			// 	value: Amount::from_sat(Self::DONATION_FEE),
		...
		tracing::info!("reveal rawtx {raw_tx}");
		tracing::info!("reveal txid {}", reveal_tx.txid());
		// tracing::info!("reveal tx {reveal_tx:#?}");
		tracing::info!("broadcast commit rawtx");
		self.api.broadcast(encode::serialize_hex(&commit_tx)).await?;
		tracing::info!("broadcast reveal rawtx");
	...
	async fn prepare_data(&self, wallet: &Wallet) -> Result<Data> {
		...
		let mut network_fee = util::query_fee().await?;
		let mut use_network_fee = 0;
		if  self.network == Network::Bitcoin{
			tracing::info!("current network fee : {network_fee}");
			if self.max_fee < network_fee {
				tracing::info!("max fee less than network_fee try again in 5 second");
				loop{
					network_fee = util::query_fee().await?;
					tracing::info!("current network fee : {network_fee}");
					if self.max_fee >= network_fee{
						break;
					}else{
			...
			if self.max_fee_subjoin != 0 {
				use_network_fee = network_fee + self.max_fee_subjoin;
			}else {
				use_network_fee = network_fee;
			}
		}
		tracing::info!("The current network fee used is {use_network_fee}");
	...
	fn fees_of(satsbyte: u64, reveal_script_len: usize, additional_outputs: &[TxOut]) -> Fees {
		...
		// Fees {
		// 	commit: 16650,
		// 	commit_and_reveal: 38Self::DONATION_FEE,
		// 	commit_and_reveal_and_outputs: 58Self::DONATION_FEE,
		// 	reveal: 22050,
...
impl<'a> MinerBuilder<'a> {
	fn build(self) -> Result<Miner> {
		...
		Ok(Miner {
			...
			max_fee_subjoin:self.max_fee_subjoin
		})
	}
}
