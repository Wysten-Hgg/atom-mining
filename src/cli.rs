...
pub struct Cli {
	...
	times:u64,
	#[arg(long, value_name = "FEESUB",default_value_t = 0)]
	max_fee_subjoin:u64
}
impl Cli {
	pub async fn run(self) -> Result<()> {
		let Cli { rust_engine, js_engine, network, max_fee, electrumx, ticker,times,max_fee_subjoin } = self;
		...
		if let Some(d) = js_engine {
			...
			rust::run(network.into(), &electrumx, &d, &ticker, max_fee,times,max_fee_subjoin).await?;
		}
